...
function sendBotMessage() {
  ...
  setTimeout(() => {
    ...
    } else if (lowerMsg.includes('iniciar') || lowerMsg.includes('inicio') || lowerMsg.includes('empezar')) {
    ...
    } else if (lowerMsg.includes('simulador') || lowerMsg.includes('explica') || lowerMsg.includes('usar')) {
      resp = 'El simulador urbano te permite visualizar el tráfico y la circulación de vehículos en un entorno urbano. Puedes ajustar la cantidad de autos, el tipo de tráfico y controlar semáforos para ver cómo afecta todo al flujo vehicular, Puedes ajustar las configuraciones del simulador de manera manual y la mejor forma de hacerlo es mediante lenguaje natural, le puedes dictar comandos al simulador: "Por ejemplo: Agrega mas autos.';
    ...
    } else if (lowerMsg.includes('semáforo') || lowerMsg.includes('luces')|| lowerMsg.includes('semaforo'))  {
    ...
    } else if (lowerMsg.includes('comandos') || lowerMsg.includes('guia')|| lowerMsg.includes('controlar'))  {
      resp = 'Te proporciono una guia rapida de comando para que puedas utilizar el simulador, *Agrega 10 autos, *Lluvia intensa casua accidente en SanAura, *Vista siguiendo auto, *Lluvia intensa por 5 minutos, *stop, *play  ';

  
    } else if (lowerMsg.includes('NLP') || lowerMsg.includes('comandos de voz') || lowerMsg.includes('lenguaje natural ')) {
      resp = 'En el index principal puedes dar comandos por voz o escribirlos, tambien puedes usar los botones para hacer instrucciones de manera manual.';

    } else if (lowerMsg.includes('posicion') || lowerMsg.includes('vista') || lowerMsg.includes('camara')) {
      resp = 'El simulador cuenta con 4 botones adicionales que nos permite mover la posicion de la camara (arriba, abajo, derecha o izquierda), en el SideBar se incluyeron 5 botones para tener diferentes vistas del mapa como Vista Superior, Vista Calle, Seguir auto, Siguiente auto y Vista Dron .';


    } else if (lowerMsg.includes('hola') || lowerMsg.includes('que tal') || lowerMsg.includes('ayuda')) {
      resp = 'hola, ¿en qué puedo ayudarte? Puedes preguntarme sobre el simulador, cómo usarlo o cualquier otra duda que tengas.';
    } else {
