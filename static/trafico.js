...
const efectosClima = {
  ...
  lluvia: {
    leve: {
      colorAmbiente: 0xcccccc,
      intensidadLuz: 0.8,
      visibilidad: 0.9,
      factorVelocidad: 0.9,
    },
    moderado: {
      colorAmbiente: 0xaaaaaa,
      intensidadLuz: 0.6,
      visibilidad: 0.7,
      factorVelocidad: 0.8,
    },
    fuerte: {
      colorAmbiente: 0x888888,
      intensidadLuz: 0.4,
      visibilidad: 0.5,
      factorVelocidad: 0.7,
    },
  ...
  niebla: {
    leve: {
      colorAmbiente: 0xdddddd,
      intensidadLuz: 0.7,
      visibilidad: 0.8,
      factorVelocidad: 0.95,
    },
    moderado: {
      colorAmbiente: 0xbbbbbb,
      intensidadLuz: 0.5,
      visibilidad: 0.6,
      factorVelocidad: 0.9,
    },
    fuerte: {
      colorAmbiente: 0x999999,
      intensidadLuz: 0.3,
      visibilidad: 0.4,
      factorVelocidad: 0.85,
    },
  },
  noche: {
    leve: {
      colorAmbiente: 0x444444,
      intensidadLuz: 0.3,
      visibilidad: 0.6,
      factorVelocidad: 0.9,
    },
    moderado: {
      colorAmbiente: 0x333333,
      intensidadLuz: 0.2,
      visibilidad: 0.4,
      factorVelocidad: 0.8,
    },
    fuerte: {
      colorAmbiente: 0x222222,
      intensidadLuz: 0.1,
      visibilidad: 0.3,
      factorVelocidad: 0.7,
    },
...
let vistasConfig = {
  ...
  aerea: {
    position: new THREE.Vector3(50, 200, 150),
...
// Función para crear un edificio de vidrio azul con efecto de ventanas
function crearEdificio(ancho, alto, profundidad, x, y, z) {
  const geometry = new THREE.BoxGeometry(ancho, alto, profundidad);

  // Creamos un canvas dinámico para simular ventanitas
  ...
  // Dibujar cuadrícula de ventanitas amarillas/grises
  for (let i = 10; i < 256; i += 20) {
    for (let j = 10; j < 256; j += 20) {
      ctx.fillStyle = Math.random() > 0.7 ? "#ffd700" : "#9eceffff";
  ...
  texture.repeat.set(4, 8); // más repeticiones para dar efecto

  const material = new THREE.MeshStandardMaterial({
    map: texture,
    color: 0x1e90ff,
    metalness: 0.3, // 👈 menos metálico
    roughness: 0.4, // 👈 un poco más difuso
    transparent: true,
    opacity: 0.95,
    emissive: 0x0a1f44,
  ...
  const edificio = new THREE.Mesh(geometry, material);

  // Posicionar apoyado en el suelo
  edificio.position.set(x, y + alto / 2, z);
...
function encontrarCalleEntreNodos(nodoA, nodoB) {
  for (const calleNombre in callesConNodos) {
    const calle = callesConNodos[calleNombre];
    if (
      (calle.nodoInicio === nodoA && calle.nodoFin === nodoB) ||
...
function convertirRutaNodosACoordenadas(rutaNodos) {
  ...
  for (let i = 0; i < rutaNodos.length - 1; i++) {
    ...
    const esMismaDireccion =
      calle.nodoInicio === nodoActual && calle.nodoFin === nodoSiguiente;

    if (esMismaDireccion) {
      // Agregar puntos en orden normal
      puntosRuta.push(
        ...puntosCalle.map((p) => ({ ...p, nombre: calleNombre }))
      );
    } else {
...
function encontrarRutaAlternativa(
  nodoOrigen,
  nodoDestino,
  callesBloqueadas = []
) {
...
function cambiarRutaAuto(car) {
  ...
  for (const nodoId in nodos) {
    const nodo = nodos[nodoId];
    const distancia = Math.sqrt(
      Math.pow(ultimoPunto.x - nodo.x, 2) + Math.pow(ultimoPunto.y - nodo.y, 2)
    );
  ...
  const rutaNodos = encontrarRutaAlternativa(
    nodoCercano,
    nodoDestino,
    callesBloqueadas
  );
...
async function init() {
  ...
  // Ejemplo: un rascacielos en (10,0,-15)
  const torre = crearEdificio(20, 100, 20, 10, 0, -15);
  scene.add(torre);
...
function actualizarVistaSiguienteAuto() {
  ...
  switch (vistaActual) {
    ...
    case "conductor":
      ...
      camera.position.set(carPos.x, carPos.y + 1.5, carPos.z + 0.5);
...
document.getElementById("mapaCanvas").addEventListener("click", (event) => {
  ...
  if (modoAgregarBloqueo) {
    ...
    const calleHit = intersects.find(
      (obj) => obj.object.userData && obj.object.userData.tipo === "calle"
    );
...
function simularAccidente(tipo, ubicacion, duracionMinutos) {
  ...
  aplicarEfectoAccidenteAutos(
    coordenadas,
    efecto.radio,
    efecto.reduccionVelocidad
  );
...
function buscarUbicacionEnGrafo(ubicacionTexto) {
  ...
  const textoNormalizado = ubicacionTexto
    .toLowerCase()
    .trim()
...
function animarAccidentes() {
  accidentesActivos.forEach((accidente, id) => {
    ...
    if (accidente.humo) {
      ...
      accidente.humo.position.y =
        3 + Math.sin(accidente.humo.userData.offsetY) * 2;
      accidente.humo.material.opacity =
        0.4 + Math.sin(accidente.humo.userData.offsetY * 0.5) * 0.2;
    }
...
function aplicarEfectosVisualesClima(tipo, intensidad) {
  ...
  if (tipo === "niebla" || tipo === "lluvia") {
    scene.fog = new THREE.FogExp2(
      efecto.colorAmbiente,
      0.01 * (1.5 - efecto.visibilidad)
    );
  } else {
    scene.fog = null;
  }
...
function aplicarEfectosGameplayClima(tipo, intensidad) {
  ...
  carSpeeds = carSpeeds.map(
    (originalSpeed) => originalSpeed * efecto.factorVelocidad
  );
...
function crearLluvia(intensidad) {
  const count =
    intensidad === "leve" ? 500 : intensidad === "moderado" ? 1000 : 2000;
  ...
  sistemaParticulasLluvia.userData.velocity =
    intensidad === "leve" ? 0.2 : intensidad === "moderado" ? 0.4 : 0.6;
}

function crearNiebla(intensidad) {
  const density =
    intensidad === "leve" ? 50 : intensidad === "moderado" ? 100 : 200;
...
function animarEfectosClima() {
  ...
  if (sistemaParticulasLluvia) {
    const positions =
      sistemaParticulasLluvia.geometry.attributes.position.array;
...
function cambiarVista(tipo, autoId, ubicacion) {
  ...
  mostrarNotificacion(
    `Vista cambiada a: ${tipo}${autoId ? " auto " + autoId : ""}`
  );
}
...
function siguienteAuto() {
  if (
    vistaActual === "siguiendo" ||
    vistaActual === "primera_persona" ||
